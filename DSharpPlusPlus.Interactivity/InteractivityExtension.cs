...
using DSharpPlusPlus.AsyncEvents;
using DSharpPlusPlus.Entities;
using DSharpPlusPlus.EventArgs;
using DSharpPlusPlus.Interactivity.Enums;
using DSharpPlusPlus.Interactivity.EventHandling;

namespace DSharpPlusPlus.Interactivity;

/// <summary>
/// Extension class for DSharpPlusPlus.Interactivity
/// </summary>
...
#pragma warning restore IDE1006 // Naming Styles
    ...
    /// </summary>
    /// <param name="modalId">The id of the modal to wait for. Should be unique to avoid issues.</param>
    ...
    public Task<InteractivityResult<ModalSubmittedEventArgs>> WaitForModalAsync(string modalId, TimeSpan? timeoutOverride = null)
        => WaitForModalAsync(modalId, GetCancellationToken(timeoutOverride));
    ...
    /// <returns>A <see cref="InteractivityResult{ModalSubmitEventArgs}"/> with a modal if the interactivity did not time out.</returns>
    public async Task<InteractivityResult<ModalSubmittedEventArgs>> WaitForModalAsync(string modalId, CancellationToken token)
    {
        if (string.IsNullOrEmpty(modalId) || modalId.Length > 100)
        {
            throw new ArgumentException("Custom ID must be between 1 and 100 characters.");
        }

        ModalMatchRequest matchRequest = new(modalId,
                c => c.Interaction.Data.CustomId == modalId, cancellation: token);
    ...
    /// <param name="timeoutOverride">Override the timeout period in <see cref="InteractivityConfiguration"/>.</param>
    /// <returns>A <see cref="InteractivityResult{ModalSubmitEventArgs}"/> with a modal if the interactivity did not time out.</returns>
    public Task<InteractivityResult<ModalSubmittedEventArgs>> WaitForModalAsync(string modalId, DiscordUser user, TimeSpan? timeoutOverride = null)
        => WaitForModalAsync(modalId, user, GetCancellationToken(timeoutOverride));
    ...
    /// <param name="token">A custom cancellation token that can be cancelled at any point.</param>
    /// <returns>A <see cref="InteractivityResult{ModalSubmitEventArgs}"/> with a modal if the interactivity did not time out.</returns>
    public async Task<InteractivityResult<ModalSubmittedEventArgs>> WaitForModalAsync(string modalId, DiscordUser user, CancellationToken token)
    {
        if (string.IsNullOrEmpty(modalId) || modalId.Length > 100)
        {
            throw new ArgumentException("Custom ID must be between 1 and 100 characters.");
        ...
        ModalMatchRequest matchRequest = new(modalId,
                c => c.Interaction.Data.CustomId == modalId &&
