...
using DSharpPlusPlus.Net.WebSocket;
...
public sealed partial class DiscordClient : BaseDiscordClient
{
    ...
    (
        ...
        [FromKeyedServices("DSharpPlusPlus.Gateway.EventChannel")]
        Channel<GatewayPayload> eventChannel,

        [FromKeyedServices("DSharpPlusPlus.Webhooks.EventChannel")]
        Channel<DiscordWebhookEvent> webhookEventChannel,

        [FromKeyedServices("DSharpPlusPlus.Interactions.EventChannel")]
    ...
    public async Task ConnectAsync(DiscordActivity activity = null, DiscordUserStatus? status = null, DateTimeOffset? idlesince = null)
    {
        ...
        {
            long? sinceUnix = idlesince != null ? Utilities.GetUnixTime(idlesince.Value) : null;
        ...
        this.Logger.LogInformation(LoggerEvents.Startup, "DSharpPlusPlus; version {Version}", this.VersionString);
    ...
    /// <exception cref="Exceptions.BadRequestException">Thrown when an invalid parameter was provided.</exception>
    /// <exception cref="Exceptions.ServerErrorException">Thrown when Discord is unable to process the request.</exception>
    public async Task<DiscordInvite> GetInviteByCodeAsync(string code, bool? withCounts = null)
        => await this.ApiClient.GetInviteAsync(code, withCounts);
