...
using DSharpPlusPlus.Net.InboundWebhooks.Payloads;
...
public sealed partial class DiscordClient
{
        ...
        foreach (DiscordGuild guild in guilds)
        {
            ...
            if (rawMembers != null)
            {
                foreach (JToken xj in rawMembers)
                {
                    ...
                    guild.members[xtm.User.Id] = new DiscordMember(xtm) { Discord = this, guildId = guild.Id };
                }
            }
    ...
    internal async Task OnChannelUpdateEventAsync(DiscordChannel channel)
    {
        ...
        DiscordChannel? channelNew = InternalGetCachedChannel(channel.Id, channel.GuildId);
        ...
        if (channelNew != null)
        {
            channelOld = new DiscordChannel
            {
                ...
                permissionOverwrites = [..channelNew.permissionOverwrites],
            ...
            channelNew.permissionOverwrites.Clear();
            ...
            channelNew.permissionOverwrites.AddRange(channel.permissionOverwrites);
        }
        else if (gld != null)
        {
    ...
    internal async Task OnGuildMemberUpdateEventAsync(TransportMember member, DiscordGuild guild)
    {
        ...
        DiscordMember memberAfter = new(member) { Discord = this, guildId = guild.Id };

        if (!guild.Members.TryGetValue(member.User.Id, out DiscordMember? memberBefore))
        {
            memberBefore = new DiscordMember(member) { Discord = this, guildId = guild.Id };
        }
    ...
    internal async Task OnGuildMembersChunkEventAsync(JObject dat)
    {
        ...
        {
            DiscordMember mbr = new(members[i]) { Discord = this, guildId = guild.Id };
    ...
    internal async Task OnMessageUpdateEventAsync(DiscordMessage message, TransportUser author, TransportMember member, TransportUser referenceAuthor, TransportMember referenceMember)
    {
        ...
        if (!this.MessageCache.TryGet(eventMessage.Id, out message)) // previous message was not in cache
        {
        ...
        else // previous message was fetched in cache
        {
            ...
            if (eventMessage.Flags is not null)
            {
                message.Flags = eventMessage.Flags;
            }

            message.embeds.Clear();
            message.embeds.AddRange(eventMessage.embeds);
            
            message.attachments.Clear();
            message.attachments.AddRange(eventMessage.attachments);
            
            message.Components = eventMessage.Components;
            message.Pinned = eventMessage.Pinned;
            message.IsTts = eventMessage.IsTts;
            message.Poll = eventMessage.Poll;

            // Mentions
            message.mentionedUsers.Clear();
            message.mentionedUsers.AddRange(eventMessage.mentionedUsers ?? []);
            message.mentionedRoles.Clear();
            message.mentionedRoles.AddRange(eventMessage.mentionedRoles ?? []);
            message.mentionedChannels.Clear();
            message.mentionedChannels.AddRange(eventMessage.mentionedChannels ?? []);
    ...
    {
        guild.Discord = this;
        IEnumerable<DiscordChannel> channels = channelIds.Select(x => guild.GetChannel(x) ?? new DiscordChannel { Id = x, GuildId = guild.Id }); //getting channel objects
    ...
    internal async Task OnThreadMembersUpdateEventAsync(DiscordGuild guild, ulong threadId, IReadOnlyList<DiscordThreadChannelMember> addedMembers, IReadOnlyList<ulong?> removedMemberIds, int memberCount)
    {
        DiscordThreadChannel? thread = InternalGetCachedThread(threadId, guild.Id) ?? new DiscordThreadChannel
        {
            Id = threadId,
            GuildId = guild.Id,
        };
        ...
        {
            foreach (ulong? removedId in removedMemberIds)
            {
                removedMembers.Add(guild.members.TryGetValue(removedId.Value, out DiscordMember? member) ? member : new DiscordMember { Id = removedId.Value, guildId = guild.Id, Discord = this });
            }
        ...
        {
            removedMemberIds = Array.Empty<ulong?>();
        }
    ...
    {
        DiscordGuild? guild = InternalGetCachedGuild(guildId) ?? new DiscordGuild
        {
            Id = guildId,
            Discord = this
        };
