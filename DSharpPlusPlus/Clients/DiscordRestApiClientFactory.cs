...
namespace DSharpPlusPlus.Clients;
...
public sealed class DiscordRestApiClientFactory
{
    ...
    {
        if (this.clientCache.TryGetValue($"DSharpPlusPlus.oauth-2-client:bearer-{token}", out OAuth2DiscordClient? cachedClient))
        {
        ...
        this.clientCache.Set($"DSharpPlusPlus.oauth-2-client:bearer-{token}", client, this.options);
