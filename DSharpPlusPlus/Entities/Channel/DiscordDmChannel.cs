...
public class DiscordDmChannel : DiscordChannel
{
    ...
    public async Task AddDmRecipientAsync(ulong userId, string accesstoken, string nickname)
        => await this.Discord.ApiClient.AddGroupDmRecipientAsync(this.Id, userId, accesstoken, nickname);
    ...
    /// <exception cref="Exceptions.BadRequestException">Thrown when an invalid parameter was provided.</exception>
    /// <exception cref="Exceptions.ServerErrorException">Thrown when Discord is unable to process the request.</exception>
    public async Task RemoveDmRecipientAsync(ulong userId)
        => await this.Discord.ApiClient.RemoveGroupDmRecipientAsync(this.Id, userId);
}
