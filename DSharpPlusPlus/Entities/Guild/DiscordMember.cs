...
using DSharpPlusPlus.Net.Models;
...
public class DiscordMember : DiscordUser, IEquatable<DiscordMember>
{
    ...
    /// </summary>
    [JsonIgnore]
    public string? GuildAvatarUrl => string.IsNullOrWhiteSpace(this.GuildAvatarHash) ? null : $"https://cdn.discordapp.com/{Endpoints.Guilds}/{this.guildId}/{Endpoints.Users}/{this.Id}/{Endpoints.Avatars}/{this.GuildAvatarHash}.{(this.GuildAvatarHash.StartsWith("a_") ? "gif" : "png")}?size=1024";
    ...
    /// </summary>
    [JsonIgnore]
    internal IReadOnlyList<ulong> RoleIds => this.roleIds;

    [JsonProperty("roles", NullValueHandling = NullValueHandling.Ignore)]
    internal List<ulong> roleIds;
    ...
    public DiscordVoiceState VoiceState
        => this.Discord.Guilds[this.guildId].VoiceStates.TryGetValue(this.Id, out DiscordVoiceState? voiceState) ? voiceState : null;
    ...
    public async Task TimeoutAsync(DateTimeOffset? until, string reason = default)
        => await this.Discord.ApiClient.ModifyGuildMemberAsync(this.guildId, this.Id, communicationDisabledUntil: until, reason: reason);
    ...
    public async Task SetMuteAsync(bool mute, string reason = null)
        => await this.Discord.ApiClient.ModifyGuildMemberAsync(this.guildId, this.Id, mute: mute, reason: reason);
    ...
    public async Task SetDeafAsync(bool deaf, string reason = null)
        => await this.Discord.ApiClient.ModifyGuildMemberAsync(this.guildId, this.Id, deaf: deaf, reason: reason);
    ...
    public async Task RemoveAsync(string reason = null)
        => await this.Discord.ApiClient.RemoveGuildMemberAsync(this.guildId, this.Id, reason);
    ...
    public string GetGuildAvatarUrl(MediaFormat imageFormat, ushort imageSize = 1024)
    {
        ...
        return $"https://cdn.discordapp.com/{Endpoints.Guilds}/{this.guildId}/{Endpoints.Users}/{this.Id}/{Endpoints.Avatars}/{this.GuildAvatarHash}.{stringImageFormat}?size={stringImageSize}";
    }
    ...
    public bool Equals(DiscordMember? other) => base.Equals(other) && this.guildId == other?.guildId;
