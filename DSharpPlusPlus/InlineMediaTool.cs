...
public sealed class InlineMediaTool : IDisposable
{
    ...
    private const byte WildcardByte = 0x11;
    ...
    [
        ...
        new(MediaFormat.Jpeg, [0xFF, 0xD8, 0xFF, 0xE1, WildcardByte, WildcardByte, 0x45, 0x78, 0x69, 0x66, 0x00, 0x00]),
        new(MediaFormat.Jpeg, [0xFF, 0xD8, 0xFF, 0xE0]),
        new(MediaFormat.WebP, [0x52, 0x49, 0x46, 0x46, WildcardByte, WildcardByte, WildcardByte, WildcardByte, 0x57, 0x45, 0x42, 0x50]),
        new(MediaFormat.Avif, [0x00, 0x00, 0x00, WildcardByte, 0x66, 0x74, 0x79, 0x70, 0x61, 0x76, 0x69, 0x66, 0x00, 0x00, 0x00, 0x00]),
    ...
    public InlineMediaTool(Stream stream)
    {
        ...
        if (stream.Length < MaxSignatureLength)
        {
    ...
    public MediaFormat GetFormat()
    {
        ...
        Span<byte> first16 = stackalloc byte[MaxSignatureLength];
        ...
        try
        {
            foreach (FileSignature sig in knownSignatures)
            {
                ...
                {
                    if (sig.MagicBytes[i] == WildcardByte)
                    {
                        continue;
                    }
