...
internal sealed class TransportService : ITransportService
{
    ...
    )
    {
        ...
        this.logger = factory.CreateLogger("DSharpPlusPlus.Net.Gateway.ITransportService - invalid shard");
    }
    ...
    public async ValueTask ConnectAsync(string url, int? shardId)
    {
        this.logger = shardId is null
            ? this.factory.CreateLogger("DSharpPlusPlus.Net.Gateway.ITransportService")
            : this.factory.CreateLogger($"DSharpPlusPlus.Net.Gateway.ITransportService - Shard {shardId}");
