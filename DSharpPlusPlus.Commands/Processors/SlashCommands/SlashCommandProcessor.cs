...
using DSharpPlusPlus.Commands.Converters.Results;
using DSharpPlusPlus.Commands.EventArgs;
using DSharpPlusPlus.Commands.Exceptions;
using DSharpPlusPlus.Commands.Processors.SlashCommands.ArgumentModifiers;
...
public sealed partial class SlashCommandProcessor : BaseCommandProcessor<ISlashArgumentConverter, InteractionConverterContext, SlashCommandContext>
{
    ...
    public async Task ExecuteInteractionAsync(DiscordClient client, InteractionCreatedEventArgs eventArgs)
    {
        ...
        else if (eventArgs.Interaction.Type
                is not DiscordInteractionType.ApplicationCommand
                and not DiscordInteractionType.AutoComplete
            || eventArgs.Interaction.Data.Type is not DiscordApplicationCommandType.SlashCommand)
        {
            return;
        }
    ...
    private async ValueTask<AutoCompleteContext?> ParseAutoCompleteArgumentsAsync(InteractionConverterContext converterContext)
    {
        ...
        try
        {
            ...
            while (converterContext.NextParameter())
            {
                ...
                parsedArguments.Add(converterContext.Parameter, optional.HasValue ? optional.RawValue : converterContext.Parameter.DefaultValue);
            }
        ...
        catch (Exception error)
        {
            await this.extension.commandErrored.InvokeAsync(converterContext.Extension, new CommandErroredEventArgs()
            {
                ...
                Exception = new ArgumentParseException(converterContext.Parameter, new()
                {
                    Error = error,
                    Value = converterContext.Argument?.RawValue
                }),
                CommandObject = null,
            });
