...
public class HasPermissionAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void Analyze(SyntaxNodeAnalysisContext ctx)
    {
        ...
        if (!ctx.Compilation.CheckByName(leftTypeInfo, "DSharpPlusPlus.Entities.DiscordPermission"))
        {
            return;
        }

        TypeInfo rightTypeInfo = ctx.SemanticModel.GetTypeInfo(leftBinary.Right);
        if (!ctx.Compilation.CheckByName(rightTypeInfo, "DSharpPlusPlus.Entities.DiscordPermission"))
        {
            return;
        }
